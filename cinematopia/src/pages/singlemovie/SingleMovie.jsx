...
const SingleMovie = () => {
  ...
  const fetchReviews = async () => {
    try {
      ...
      const limitedReviews = data.results.slice(0, 2);
  ...
  return (
    <>
      ...
      <div className="movie-details px-7 pt-5">
        <div className="poster-textitems-1 flex pt-3 gap-2  text-left">
          <img
            src={imageUrl}
            alt={movieData.title}
            className="movie-poster rounded transition duration-300 ease-in-out hover:scale-105"
          ...
          <div className="movie-info text-left">
            ...
            <p className="pt-2 text-sm text-wrap">
            ...
            <p className="pt-2 flex font-bold text-xl">
              <span>
                <img src={star} className="w-6 mr-2" />
              </span>
              {movieData.vote_average}
            </p>
          </div>
          <div className="movie-review rounded ">
            <h3 className="text-left text-3xl font-bold">Reviews</h3>
            <div className="review-card">
              {reviews.map((review) => (
                <div key={review.id} className="">
                  <p className="text-left pt-2 px-2 font-bold">
                    Author: {review.author}
                  </p>
                  <p className="text-left pt-2 px-2">
                    <strong>Content:</strong> {review.content.substring(0, 200)}
                  </p>
                </div>
              ))}
            </div>
